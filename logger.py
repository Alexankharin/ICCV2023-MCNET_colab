...
class Logger:
    ...
    def log_scores(self, loss_names):
        ...
        loss_string = "; ".join(
            ["%s - %.5f" % (name, value) for name, value in zip(loss_names, loss_mean)]
        )
    ...
    def save_cpk(self, emergent=False):
        ...
        cpk_path = os.path.join(self.cpk_dir, "final-checkpoint.pth.tar")
    ...
    def load_cpk(
        ...
        if generator is not None:
            ckp_generator = collections.OrderedDict(
                (k.replace(".module.", "."), v)
                for k, v in checkpoint["generator"].items()
            )
            generator.load_state_dict(ckp_generator)
        if kp_detector is not None:
            ckp_kp_detector = collections.OrderedDict(
                (k.replace(".module.", "."), v)
                for k, v in checkpoint["kp_detector"].items()
            )
            kp_detector.load_state_dict(ckp_kp_detector)
        if avd_network is not None:
            if "avd_network" in checkpoint:
                avd_network.load_state_dict(checkpoint["avd_network"])
                # ckp_avd_network = collections.OrderedDict((k.replace('.module.','.'),v) for k,v in checkpoint['avd_network'].items())
                # avd_network.load_state_dict(ckp_avd_network)
        if discriminator is not None:
            try:
                ckp_discriminator = collections.OrderedDict(
                    (k.replace(".module.", "."), v)
        ...
        if optimizer_generator is not None:
            ckp_optimizer_generator = collections.OrderedDict(
                (k.replace(".module.", "."), v)
                for k, v in checkpoint["optimizer_generator"].items()
            )
            optimizer_generator.load_state_dict(ckp_optimizer_generator)
        if optimizer_discriminator is not None:
            try:
                ckp_optimizer_discriminator = collections.OrderedDict(
                    (k.replace(".module.", "."), v)
                    for k, v in checkpoint["optimizer_discriminator"].items()
                )
        ...
        if optimizer_kp_detector is not None:
            ckp_optimizer_kp_detector = collections.OrderedDict(
                (k.replace(".module.", "."), v)
                for k, v in checkpoint["optimizer_kp_detector"].items()
            )
    ...
    def load_cpk_bg(
        ...
        #  checkpoint, generator, kp_detector, bg_predictor, None,optimizer,
        num_gpu = torch.cuda.device_count()
        if num_gpu == 1:
            checkpoint = torch.load(checkpoint_path, map_location="cuda:0")
        else:
        ...
        if bg_predictor is not None:
            ckp_bg_predictor = collections.OrderedDict(
                (k.replace(".module.", "."), v)
                for k, v in checkpoint["bg_predictor"].items()
            )
            bg_predictor.load_state_dict(ckp_bg_predictor)
        ...
        if optimizer is not None:
            ...
            optimizer.load_state_dict(ckp_optimizer_generator)
    ...
    def visualize(self, driving, source, out):
        ...
        if "sparse_deformed" in out:
            ...
            for i in range(out["sparse_deformed"].shape[1]):
                ...
                if i != 0:
                    color = np.array(
                        self.colormap((i - 1) / (out["sparse_deformed"].shape[1] - 1))
                    )[:3]
                else:

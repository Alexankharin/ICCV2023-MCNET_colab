...
def load_checkpoints(config_path, checkpoint_path, cpu=False):
    ...
    generator = getattr(GEN, opt.generator)(
        **config["model_params"]["generator_params"],
        **config["model_params"]["common_params"],
        **{
            "mbunit": opt.mbunit,
            "mb_spatial": opt.mb_spatial,
    ...
    ckp_generator = OrderedDict(
        (k.replace("module.", ""), v) for k, v in checkpoint["generator"].items()
    )
    generator.load_state_dict(ckp_generator)
    ckp_kp_detector = OrderedDict(
        (k.replace("module.", ""), v) for k, v in checkpoint["kp_detector"].items()
    )
...
def make_animation(
    ...
    with torch.no_grad():
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            drivings.append(
                np.transpose(driving_frame.data.cpu().numpy(), [0, 2, 3, 1])[0]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint",
        default="00000099-checkpoint.pth.tar",
    ...
    parser.add_argument(
        "--mbunit", type=str, default="ExpendMemoryUnit", help="depth mode"
    )
    ...
    if opt.find_best_frame or opt.best_frame is not None:
        ...
        sources_forward, drivings_forward, predictions_forward = make_animation(
            source_image,
            driving_forward,
            generator,
            kp_detector,
        ...
        sources_backward, drivings_backward, predictions_backward = make_animation(
            source_image,
            driving_backward,
            generator,
            kp_detector,
    ...
    imageio.mimsave(
        opt.result_video,
        [
            np.concatenate((img_as_ubyte(s), img_as_ubyte(d), img_as_ubyte(p)), 1)
            for (s, d, p) in zip(sources, drivings, predictions)
        ],
        fps=fps,
    )

...
from modules.dynamic_conv import (
    Dynamic_conv2d,
    Multi_Dynamic_conv2d,
    Dynamic_conv2d_conditional,
)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, source, feat):
        ...
        proj_query = (
            self.activation(self.query_conv(source))
            .view(m_batchsize, -1, width * height)
            .permute(0, 2, 1)
        )  # B X CX(N) [bz,32,64,64]
        proj_key = self.activation(self.key_conv(feat)).view(
            m_batchsize, -1, width * height
        )  # B X C x (*W*H)
        energy = torch.bmm(proj_query, proj_key)  # transpose check
        attention = self.softmax(energy)  # BX (N) X (N)
        proj_value = self.activation(self.value_conv(feat)).view(
            m_batchsize, -1, width * height
        )  # B X C X N
...
class MemoryUnit(nn.Module):
    ...
    def __init__(self, spatial_dim, channel_dim):
        ...
        self.mb = torch.nn.Parameter(
            torch.randn(1, channel_dim, spatial_dim, spatial_dim)
        )
        ...
        self.query_conv = nn.Conv2d(
            in_channels=channel_dim, out_channels=channel_dim, kernel_size=1, padding=0
        )

        self.conv_kv = Dynamic_conv2d(
            in_planes=channel_dim,
            out_planes=channel_dim * 2,
            kernel_size=3,
            ratio=0.25,
            padding=1,
            groups=1,
    ...
    def forward(self, feature, output_dict):
        ...
        q, k, v = map(
            lambda t: rearrange(t, "b c x y -> b (x y) c"), (query, key, value)
        )
...
class ExpendMemoryUnit(nn.Module):
    ...
    def __init__(self, spatial_dim, channel_dim, half=True):
        ...
        self.feat_forward_proj_32 = nn.Conv2d(
            in_channels=self.channel_32,
            out_channels=channel_dim,
            kernel_size=1,
            padding=0,
        )
        self.feat_forward_proj_64 = nn.Conv2d(
            in_channels=self.channel_64,
            out_channels=channel_dim,
            kernel_size=1,
            padding=0,
        )
        self.feat_forward_proj_128 = nn.Conv2d(
            in_channels=self.channel_128,
            out_channels=channel_dim,
            kernel_size=1,
            padding=0,
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        for i in range(num_down_blocks):
            up_blocks.append(
                UpBlock2d(
                    in_features[i], out_features[i], kernel_size=(3, 3), padding=(1, 1)
                )
            )
        ...
        for i in range(num_down_blocks):
            resblock.append(
                ResBlock2d(in_features[i], kernel_size=(3, 3), padding=(1, 1))
            )
            resblock.append(
                ResBlock2d(in_features[i], kernel_size=(3, 3), padding=(1, 1))
            )
        ...
        print(kwargs["mbunit"])
        print("AAAAAAAAAAA")
        print(eval(kwargs["mbunit"]))
        print("BBBBBBBBBBB")
        print(kwargs["mb_spatial"], kwargs["mb_channel"])
